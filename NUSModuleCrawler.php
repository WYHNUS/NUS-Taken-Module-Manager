...
class NUSModuleCrawler {
    // information to login to myisis
    private $matric_num;
    private $pwd;

    // url to retrive all exam result root directory
    private $all_result_url = "https://myisis.nus.edu.sg/psc/cs90prd/EMPLOYEE/HRMS/c/SA_LEARNER_SERVICES.N_SSR_SSENRL_GRADE.GBL?FolderPath=PORTAL_ROOT_OBJECT.CO_EMPLOYEE_SELF_SERVICE.HCCC_ENROLLMENT.N_SSR_SSENRL_GRADE_GBL";

    // url to retrieve specified semester exam result
    private $sem_result_url = "https://myisis.nus.edu.sg/psc/cs90prd/EMPLOYEE/HRMS/c/SA_LEARNER_SERVICES.N_SSR_SSENRL_GRADE.GBL";
    private $sem_result_field = "ICAJAX=1&ICAction=N_DERIVED_EXAM_SSR_PB_GO&SSR_DUMMY_RECV1\$sels\$0=";

    public function __construct($matric_num, $pwd) {
        $this->matric_num = $matric_num;
        $this->pwd = $pwd;
    }

    public function run() {
        $cookiefile = realpath("cookie");
        $ch = curl_init();

        $login_state = $this->login($ch, $this->matric_num, $this->pwd, $cookiefile);
        ...
        if ($login_state && !strpos(strip_tags($login_state), "Your User ID and/or Password are invalid")) {
            ...
            $all_result_page = $this->curl_get($ch, $this->all_result_url, $cookiefile);
            // parse dom tree
            ...
            for ($i=0; $i<count($sem_index_array); $i++) {
                $sem_result_page = $this->curl_post($ch, $this->sem_result_url, $this->sem_result_field.$sem_index_array[$i], array(CURLOPT_COOKIEFILE => $cookiefile));
                
                // logout then login again to avoid cached content 
                // (there must exist a better way to do this...)
                $this->logout($ch);
                $this->login($ch, $this->matric_num, $this->pwd, $cookiefile);
                $this->curl_get($ch, $this->all_result_url, $cookiefile);
    ...
    private function curl_post($curl, $url, $post_fields, $add_opt=array()) {
    ...
    private function curl_get($curl, $url, $cookiefile) {
        $options = array(
            CURLOPT_URL => $url,
            CURLOPT_RETURNTRANSFER => TRUE,
    ...
    private function login($ch, $matric_num, $pwd, &$cookiefile) {
        ...
        return $this->curl_post($ch, $login_url, $login_fields, array(CURLOPT_COOKIEJAR => $cookiefile));
    }

    private function logout($ch) {
        $logout_url = "https://myisis.nus.edu.sg/psp/cs90prd/EMPLOYEE/HRMS/?cmd=logout";
        return $this->curl_get($ch, $logout_url, $cookiefile);
    }

    private function parse_module($module_text) {
        $modules_taken = array();
